...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        ...
        if name == "LasHeR_train":
            datasets.append(LasHeR_trainingSet(settings.env.lasher_train_dir))
        if name == "LasHeR_test":
            datasets.append(LasHeR_testingSet(settings.env.lasher_test_dir))
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_cls:
        ...
        param_dicts = [
            # {"params": [p for n, p in net.named_parameters() if "tbsi" in n and p.requires_grad],               # tbsi_fuse
            # },
            # {                                                                                                   # backbone
            #     "params": [p for n, p in net.named_parameters() if "tbsi" not in n and p.requires_grad],
            #     "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
            # },
            # {"params": [p for n, p in net.named_parameters() if "tbsi_fuse" in n and p.requires_grad],               # tbsi_fuse
            # },
            {"params": [p for n, p in net.named_parameters() if (("tbsi_fuse" in n) or ("pcf" in n) or ("feadnet" in n)) and p.requires_grad],               # tbsi_fuse  feadnet
            },
            {                                                                                                   # backbone
                "params": [p for n, p in net.named_parameters()  if ("tbsi_fuse" not in n) and ("pcf" not in n) and ("feadnet" not in n) and p.requires_grad],
                "lr": cfg.TRAIN.LR * 0.1,
                # "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
            },
        ]
        if is_main_process():               # do
