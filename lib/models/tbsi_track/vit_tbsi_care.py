...
from lib.models.layers.FeaDENet import FeaDENet
from lib.models.layers.crossAttention import crossA
...
class VisionTransformerTBSI(BaseBackbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 tbsi_loc=None, tbsi_drop_path=None, hhb_loc=None,fdnet_loc=None, pcf_loc=None):
        """
        Args:
        ...
        self.alpha = nn.Parameter(torch.ones(1) * 0.00001, requires_grad=False)
        ...
        lambda_list = [0.15, 0.15, 0.2, 0.2, 0.25, 0.25, 0.3, 0.3, 0.35, 0.35, 0.4, 0.4]
        
        self.fdnet_loc = fdnet_loc if fdnet_loc is not None else []
        print("self.fdnet_loc",self.fdnet_loc)
        self.feadnet_layers = nn.ModuleList([
            FeaDENet(embed_dim=embed_dim, tmpl_hw=(8, 8), search_hw=(16, 16)) for loc in self.fdnet_loc
        ])
        self.pcf_crossA_v = crossA(768, norm_layer=norm_layer)
        self.pcf_crossA_i = crossA(768, norm_layer=norm_layer)

        self.pcf_block =  Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
    ...
    def forward_features(self, z, x):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in self.fdnet_loc:
                # print(i)
                x_v, x_i = self.feadnet_layers[fdnet_index](x_v, x_i, lens_z)
                fdnet_index += 1
        ...
        # def forward(self, initial, dynamic):
        v_token = self.pcf_crossA_v(x_v[:, :64, :], x_v[:, :64, :])
        i_token = self.pcf_crossA_i(x_i[:, :64, :], x_i[:, :64, :])

        # print("v_token.shape", v_token.shape)       # v_token.shape torch.Size([32, 64, 768])

        x_v = torch.cat([x_v, v_token], dim=1)
        x_i = torch.cat([x_i, i_token], dim=1)

        x_v = self.pcf_block(x_v)
        x_i = self.pcf_block(x_i)
...
def vit_base_patch16_224_ce_adapter(pretrained=False, **kwargs):
    ...
    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_224_ce_adapter', pretrained=pretrained, **model_kwargs)
    return model
